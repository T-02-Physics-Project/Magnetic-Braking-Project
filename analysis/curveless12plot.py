...
for measurement in angular_velocity_headers:
    ...
    if sum(valid1)>2:
        ...
        if sum(valid3)<2:
            times2 = np.linspace(0,1+max(data.t),1000)
            ...
            times2 = np.linspace(0, 1+max(data.t[valid3]), 1000)
            times3 = np.linspace(min(data.t[valid3])-1, 1+max(data.t), 1000)
...
fig.savefig('magbreak12plotsnofit.png')
...
while i<n:
    if grad2[i]!=-10:
        print("For set", angular_velocity_headers[i-1],", the log of the data takes the form of two linear plots, defined as {:n} t + {:n} and {:n} t + {:n}.\n" .format(grad[i],intercept[i],grad2[i],intercept2[i]))
        print("The data can then be plotted as two exponential plots, defined as e to the power of ({:n} t + {:n}) and e to the power of ({:n} t + {:n}).\n\n" .format(a1[i],b1[i],a2[i],b2[i]))
    else:
        print("For set", angular_velocity_headers[i-1],", the log of the data takes the form of a linear plot, defined as {:n} t + {:n}.\n".format(grad[i],intercept[i]))
